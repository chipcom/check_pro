        ...
        if (oNode1 := oXmlNode:Find('PLACES')) != NIL
          for j1 := 1 TO Len( oNode1:aItems )
            ...
            if 'PLACE' == oNode2:title
              if (year(xml2date(read_xml_stroke_1251_to_utf8(oNode2, 'D_E',))) >= val(work_year)) .or. empty(str(year(xml2date(read_xml_stroke_1251_to_utf8(oNode2, 'D_E',))), 4))
                select SD
                sd->(dbappend())
  ...
  if Empty( oXmlDoc:aItems )
    ...
    FOR j := 1 TO k
      ...
      if 'ZGLV' == oXmlNode:title
        if ! ((val(j1 := read_xml_stroke_1251_to_utf8(oXmlNode, 'YEAR_REPORT',))) == val(work_year))
                    ...
                    if (oNode5 := oNode4:Find('PAY_LEVELS')) != NIL
                      for j3 := 1 TO Len( oNode5:aItems )
                        ...
                        if 'PAY_LEVEL' == oNode6:title
                          if year(xml2date(read_xml_stroke_1251_to_utf8(oNode6, 'D_E',))) == val(work_year)
                            lp->(dbAppend())
...
//       oXmlNode := oXmlDoc:aItems[1]:aItems[j]
//       if 'ZGLV' == oXmlNode:title
//         if !((j1 := read_xml_stroke_1251_to_utf8(oXmlNode, 'YEAR_REPORT',)) == CURENT_YEAR)
//           out_error(fError, TAG_YEAR_REPORT, nfile, j1)
//           exit
//         endif
...
//                     endif
//                     if (str(year(xml2date(read_xml_stroke_1251_to_utf8(oNode4, 'D_E', ))), 4) >= CURENT_YEAR) .or. empty(str(year(xml2date(read_xml_stroke_1251_to_utf8(oNode4, 'D_E', ))), 4))
//                       select MS
//                       append blank
