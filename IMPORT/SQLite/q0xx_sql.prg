...
function make_q015(db, source, fOut, fError)
  ...
  if ! hb_vfExists(nfile)
    ...
    fOut:add_string(hb_eol() + nameRef + ' - Перечень технологических правил реализации ФЛК в ИС ведения персонифицированного учета сведений об оказанной медицинской помощи (FLK_MPF)' + hb_eol())
  endif
...
function make_q016(db, source, fOut, fError)
  ...
  if ! hb_vfExists(nfile)
    ...
    fOut:add_string(hb_eol() + nameRef + ' - Перечень проверок автоматизированной поддержки МЭК в ИС ведения персонифицированного учета сведений об оказанной медицинской помощи (MEK_MPF)' + hb_eol())
  endif
...
function make_q017(db, source, fOut, fError)
  ...
  if ! hb_vfExists(nfile)
    ...
    fOut:add_string(hb_eol() + nameRef + ' - Перечень категорий проверок ФЛК и МЭК (TEST_K)' + hb_eol())
  endif
